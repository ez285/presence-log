...
class PresenceLog:
    ...
    def LogPresence(self, data:list[dict[str, int|float|str]]) -> None:
        ...
        self.Sheets.PresenceLog.append_rows([[0, '', itm['Company ID'], itm['Person ID'], itm['In'].isoformat(), itm['Out'].isoformat()] for itm in data], insert_data_option=gs.utils.InsertDataOption.insert_rows, table_range='A1') # type: ignore
